...
- **DAVINCI-MONET promotions deferred.** The original spec called for a thin wrapper around `davinci_monet.observations.surface.aeronet.AERONETReader` and listed monthly-EBAF + regional-mask helpers as reusable extractions toward DAVINCI-MONET's `ceres` branch. The real-data AERONET adapter (per-month files, hourly→monthly resample, -1.0 fill-mask, time-broadcast site-coord collapse, bbox + siteid filtering, aod_500nm→aeronet_aod_550 rename) ended up substantial enough to land in FIREX rather than back-promote preemptively. Worth revisiting once the prototype has been used on a second region: which pieces (a) generalize cleanly, (b) belong on the `ceres` branch, (c) want their own davinci_monet-side tests. Same question for the EBAF loader (lon-wrap + var-name map) and the bbox-mask + cosine-area-weight helper.
